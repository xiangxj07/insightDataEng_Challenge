...
def processQuery(stream, net, degrees, outFiles):
	'''
	Process the queries in the file, and write results to the output files
	---------------
	Parameter:
	stream: the file containing all the transaction to be queried
	net: the friendship network, updated during the run and returned
	degrees: list of degree of friendship that is allowed
	outFiles: list of output file names
	'''
	if len(outFiles) != len(degrees):
		warnings.warn('The output file number is not the same as degree list!')
		return
	#open files listed in the input
	for i in range(0, len(outFiles)):
		op = "fid" + str(i+1) + "=" + "open(outFiles[i], 'w')"
		exec(op)
	print 'Now checking the coming transactions and writing to output...'
	#send the queries in the stream file
	count = 0
	for pair in stream:
		count += 1
		#evaluate the fraud
